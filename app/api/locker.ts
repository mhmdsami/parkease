...
export const openLockerApi = async (token: string) => {
  const res = await fetch(
    API.BASE_URL +
      API.ENDPOINTS.LOCKER.BASE_URL() +
      API.ENDPOINTS.LOCKER.OPEN(),
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
...
export const closeLockerApi = async (token: string) => {
  const res = await fetch(
    API.BASE_URL +
      API.ENDPOINTS.LOCKER.BASE_URL() +
      API.ENDPOINTS.LOCKER.CLOSE(),
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
  ...
  if (!res.ok) {
    throw new Error("Failed to close locker");
  }
